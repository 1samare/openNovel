...
工作流通过 npm 脚本执行质量检查，不承载部署、发布或密钥管理逻辑。Node 版本不得低于 `22.13.0`，以确保 `node:sqlite` 无需实验开关即可加载。
...
- 2026-08-10：将 Windows Quality 的 Node 下限提升到 `22.13.0`，修复 `node:sqlite` 在 22.12 中未默认开放导致的测试启动失败。
