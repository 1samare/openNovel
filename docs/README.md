# docs 目录说明

## 目录用途

按执行日期归档架构设计、修改计划和验证结果，为人类开发者与 Harness Agent 提供可追溯的实施依据。

## 内容说明

- 日期目录统一使用 `YYYY-MM-DD`。
- 每项独立修改使用一份以 `计划.md` 结尾的实施计划。
- 架构设计与同一任务的计划、结果放在同一日期目录。
- Harness Agent 离线闭环计划包含分任务 TDD、生产验收、偏差和最终审查记录。
- `Electron生产冒烟自动化计划.md`：记录生产 smoke、Windows CI、脱敏诊断和实际验证结果。
- `2026-08-06/`：记录多 Agent 小说创作助手架构、正文质量门禁、BYOK 模型与外部 Skill 安全边界。
- `2026-08-07/`：记录从现有 Harness 到 Windows 可安装小说创作软件的全阶段开发计划。
- `2026-08-10/`：记录阶段 2 章节编辑、版本与本地文件交换的开工基线、实施计划和验证结果。
- `2026-08-11/`：记录阶段 4 小说圣经、结构化共创、审批版本和世界观/人物/大纲页面的实施计划与验证结果。
- 根目录 `DEVELOPMENT_PROGRESS.md`：作为各阶段计划的交接入口，记录工作区、分支、阶段状态、验证和下一步。

## 依赖边界

本目录只保存项目文档，不承载运行时代码、生成产物或本地密钥。

## 维护规则

新增或修改日期目录中的文档时，必须同步更新该日期目录的 `README.md`；修改本文件时同步更新根目录 `README.md`。

## 变更同步

- 2026-07-28：建立文档目录契约并登记 Harness Agent 离线闭环设计与计划。
- 2026-07-28：完成 README 目录契约、检查脚本和 Windows CI 基础，并记录验证结果。
- 2026-07-28：回填 Harness Agent Task 2 的公共契约、运行时校验、状态机和验证结果。
- 2026-07-28：记录 Task 2 Fix Round 1 的安全错误规范化修复与验证结果。
- 2026-07-28：记录 Task 5 的安全 Agent IPC、Preload 最小桥接、运行时恢复和脱敏日志实现。
- 2026-07-28：登记 Task 5 Fix Round 1 的安全桥生命周期与 sender 边界复审修复计划。
- 2026-07-28：回填 Task 5 Fix Round 1 的生命周期与 sender 边界验证结果。
- 2026-07-28：登记 Task 5 Fix Round 1 Minor 的 file sender URL 规范化修复计划。
- 2026-07-28：回填 Task 5 Fix Round 1 Minor 的 production file hash 路由验证结果。
- 2026-07-28：回填 Task 6 的 Harness Agent 渲染闭环、事件回补与质量验证结果。
- 2026-07-28：登记 Task 6 Fix Round 1 的 Harness UI 异步状态、重试和可访问性加固计划。
- 2026-07-28：回填 Task 6 Fix Round 1 的 Harness UI 异步状态、重试、可访问性与响应式验证结果。
- 2026-07-28：登记 Task 6 Fix Round 1 复审收尾的失败详情回读和窄屏顶栏命中区验证。
- 2026-07-28：回填 Task 7 的真实 Electron 闭环验收、三项生产验证修复和最终质量门禁记录。
- 2026-07-28：登记 Electron 生产冒烟自动化、Windows CI 门禁和脱敏诊断计划。
- 2026-08-07：登记多 Agent 小说创作助手架构设计和九阶段开发计划。
- 2026-08-07：登记开发进度清单及阶段开发前强制读取、核对和写回的协作约束。
- 2026-08-10：登记阶段 2 当天计划、开工基线和 TDD/退出门禁。
- 2026-08-10：回填阶段 2 两轮独立复审、最终全门禁与待用户确认交接结果。
- 2026-08-11：登记阶段 4 小说圣经与结构化共创实施计划、开工基线和本地 TDD 门禁。
